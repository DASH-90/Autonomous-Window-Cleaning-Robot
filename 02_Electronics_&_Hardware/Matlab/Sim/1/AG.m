...
% time span
tspan=[0 4]; % Extended time span for full motion

% input torques (initial straight motion)
T1=20;
T2=20;
...
for ii=2:length(T)
    % Update torques for turning
    if x(ii) > 3 && x(ii) < 6
        T1 = 20;
        T2 = 10; % Change torques to turn the robot
    else
        T1 = 20;
        T2 = 20; % Straight motion before and after the turn
    end
